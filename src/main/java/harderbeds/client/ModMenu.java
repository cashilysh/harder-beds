    ...
    public ConfigScreenFactory getModConfigScreenFactory() {
        return parent -> {
            ConfigBuilder builder = ConfigBuilder.create()
                    ...
                    .setTitle(Component.literal("Harder Beds Configuration"));
            ...
            mainCategory.addEntry(
                    entryBuilder.startBooleanToggle(
                                    Component.literal("Simulate Mob Pathing Before Sleeping"),
                                    settings.shouldSimulateMobPathingOnSleep()
                            )
                            .setDefaultValue(defaults.shouldSimulateMobPathingOnSleep())
                            .setTooltip(Component.literal("If enabled, the game will check if hostile mobs can reach the bed before allowing you to sleep."))
            ...
            mainCategory.addEntry(
                    entryBuilder.startBooleanToggle(
                                    Component.literal("Visualize Mob Path"),
                                    settings.isMobPathVisualizationEnabled()
                            )
                            .setDefaultValue(defaults.isMobPathVisualizationEnabled())
                            .setTooltip(Component.literal("Shows the simulated mob path to the bed (if a path is found) using particle effects. Requires path simulation to be enabled."))
            ...
            mainCategory.addEntry(
                    entryBuilder.startBooleanToggle(
                                    Component.literal("Prevent Bed Drop in Villages"),
                                    settings.shouldPreventBedDropInVillages()
                            )
                            .setDefaultValue(defaults.shouldPreventBedDropInVillages())
                            .setTooltip(Component.literal("If enabled, beds located within a village structure will not drop as an item when mined."))
            ...
            mainCategory.addEntry(
                    entryBuilder.startBooleanToggle(
                                    Component.literal("Apply Penalty for Using Village Beds"),
                                    settings.isVillageBedPenaltyEnabled()
                            )
                            .setDefaultValue(defaults.isVillageBedPenaltyEnabled())
                            .setTooltip(Component.literal("If enabled, sleeping in a bed located within a village will apply negative potion effects to the player."))
            ...
            mainCategory.addEntry(
                    entryBuilder.startBooleanToggle(
                                    Component.literal("Disable Phantom Spawning"),
                                    settings.isPhantomSpawningDisabled()
                            )
                            .setDefaultValue(defaults.isPhantomSpawningDisabled())
                            .setTooltip(Component.literal("If enabled, no phantoms will spawn even if the player doesnt sleep."))
